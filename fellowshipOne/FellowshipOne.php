<?php 
	...
	class FellowshipOne{
		...
		public function listStatuses() {
			$url = $this->settings->baseUrl . '/v1/People/Statuses.json';
			$statusArray = $this->fetchGetJson($url);
			return $statusArray['statuses']['status'];						
		}
		 ...
		 * @param int $personId
		 */
		public function createHouseholdAddress($model,$hhId){
			$url = str_replace('{householdID}',$hhId,$this->settings->baseUrl . $this->paths['people']['createHouseholdAddress'] . ".json");
			$model = json_encode($model);
			return $this->fetchPostJson($url,$model);
		}		
		
		public function getPersonCommunications($personId) {
			$url = str_replace('{personID}',$personId,$this->settings->baseUrl . $this->paths['people']['listCommunication'] . ".json");
			$model = json_encode($model);
			return $this->fetchPostJson($url,$model);			
		}
		...
		public function listAttributeGroups() {
			...
			return $groups['attributeGroups']; 
		}
		
		public function getAttributeModelByPerson($id) {
			$url = $this->settings->baseUrl . '/v1/People/' . $id . '/Attributes/new.json';
			return $this->fetchGetJson($url);
		}
		
		public function setAttribute($model) {
			$url = $this->settings->baseUrl . '/v1/People/' . $model['attribute']['person']['@id'] . '/Attributes';
			return $this->fetchPostJson($url,json_encode($model));
		}
		...
		/**
		 * fetch household communication model from F1
		 * @param int $householdId
		 */
		public function getHouseholdAddressModel($householdId){
			$url = str_replace('{householdID}',$householdId, $this->settings->baseUrl . $this->paths['people']['newHouseholdAddress'] . ".json");
			return $this->fetchGetJson($url);
		}		
		...
		*/
		public function putRequest($url, $requestBody = "", $successHttpCode = 200){
			$request = OAuthRequest::from_consumer_and_token($this->consumer, $this->token, 'PUT', $url, NULL);
			$request->sign_request(new OAuthSignatureMethod_HMAC_SHA1(), $this->consumer, $this->token);			
			$response = $this->sendRequest("PUT", trim($url),$requestBody);
			return $this->parseResponseJson($response);
		}
